  ...
  Widget _buildReceiptList() {
    return FutureBuilder<bool>(
      ...
      builder: (_, snapshot) {
        ...
        // 3. ✅ APPLY FILTERS FIRST (before batching)
        ...
        // 4. ✅ BATCH THE FILTERED RESULTS
        final total = filtered.length;
        final end = _currentBatchEnd.clamp(0, total);
        final pagedReceipts = filtered.sublist(0, end);
        print("📑 Batching: showing $end of $total");

        if (pagedReceipts.isEmpty) {
          return const Center(child: Text("No receipts found"));
        }

        return Column(
          children: [
            // SELECT ALL (based on filtered results)
            Row(
              children: [
                Checkbox(
                  value: pagedReceipts.isNotEmpty &&
                      _selectedIndexes.length == pagedReceipts.length,
                  onChanged: (checked) {
                    setState(() {
                      if (checked == true) {
                        _selectedIndexes
                          ..clear()
                          ..addAll(
                              List.generate(pagedReceipts.length, (i) => i));
                      } else {
            ...
            // PAGINATION (using filtered total)
            Row(
            ...
            Expanded(
              child: ListView.builder(
                itemCount: pagedReceipts.length,
                itemBuilder: (context, index) {
                  final log = pagedReceipts[index];
