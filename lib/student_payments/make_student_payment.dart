...
// Simple in-memory cache for server search results
  ...
  Future<void> fetchUsers() async {
    try {
      debugPrint('ℹ️ Starting fetchUsers()...');

      _role = await getDeviceRole();
      debugPrint('ℹ️ Device role: $_role');

      final prefs = await SharedPreferences.getInstance();
      _hostIp = prefs.getString('host_ip') ?? '192.168.8.2';
      debugPrint('ℹ️ Host IP: $_hostIp');
      ...
      if (_role == DeviceRole.host) {
        ...
        //
        debugPrint('ℹ️ Host device detected: loading users from Hive...');
        final userBox = await Hive.openBox<User>('users');
        allUsers = userBox.values.toList();
        debugPrint('✅ Loaded ${allUsers.length} users from Hive');
      } else {
        //
        // ------------------ CLIENT: Fetch users from host ------------------
        //
        debugPrint('ℹ️ Client device detected: fetching users from host...');
        if (_hostIp!.isEmpty) {
          debugPrint('⚠️ Host IP not set. Cannot fetch users.');
        ...
        if (_cachedServerUsers != null) {
          debugPrint(
              'ℹ️ Using cached server users: ${_cachedServerUsers!.length} users');
          allUsers = _cachedServerUsers!;
        } else {
          debugPrint('ℹ️ No cached users, performing HTTP request...');
          final usersResponse = await HttpClient()
              .getUrl(Uri.parse('http://$_hostIp:8080/api/users'))
              .then((req) => req.close());

          debugPrint(
              'ℹ️ HTTP GET /api/users status: ${usersResponse.statusCode}');

          if (usersResponse.statusCode == 200) {
            final usersJsonString =
                await usersResponse.transform(utf8.decoder).join();
            debugPrint(
                'ℹ️ Received users JSON (truncated 200 chars): ${usersJsonString.substring(0, usersJsonString.length > 200 ? 200 : usersJsonString.length)}');
            ...
            allUsers = _cachedServerUsers!;
            debugPrint(
                '✅ Fetched and parsed ${allUsers.length} users from host API');
          } else {
            throw Exception(
      ...
      if (allUsers.isNotEmpty) {
        ...
        debugPrint('✅ Populated _usersMap with ${_usersMap.length} entries');
      } else {
        _users = [];
        _usersMap = {};
      ...
      debugPrint('ℹ️ fetchUsers() completed successfully.');
      setState(() {});
    } catch (error, stack) {
...
// SAVE RECEIPT IN HIVE
                    await saveReceiptLog(
                      receiptNumber: newId,
                      student: _selectedStudent!,
                      receiptLines: list,
                    );
                    // Finish payment upload
                    _makePayment();
                  },
...
// SAVE RECEIPT IN HIVE
  ...
  Future<void> saveReceiptLog({
    ...
    if (phone.isEmpty)
      debugPrint("⚠️ Student phone missing for ${student.name}");
    if (parentName.isEmpty)
      debugPrint("⚠️ Parent name missing for ${student.name}");
    ...
    if (role == DeviceRole.host) {
      ...
      await box.add(log);
      debugPrint(
          "✅ HOST: Saved PaymentLog #$receiptNumber for ${student.name}");
    } else {
      // ----------------------------
      // CLIENT → Send to host via Alfred API
      // ----------------------------
      try {
        final prefs = await SharedPreferences.getInstance();
        final hostIp = prefs.getString('host_ip') ?? '192.168.8.2';
        ...
        if (response.statusCode == 200) {
          debugPrint(
              "✅ CLIENT: Sent PaymentLog #$receiptNumber to host successfully");
        } else {
          debugPrint(
              "❌ CLIENT: Failed to send log to host: ${response.statusCode} ${response.body}");
        }
      } catch (e) {
      ...
      try {
        ...
        if (response.statusCode == 200) {
          if (termAggregation) {
            ...
            if (phone.isNotEmpty) {
              await _sendSmsNotification(allPaymentsInfo, phone);
              await Future.delayed(const Duration(milliseconds: 800));
            }

            if (phone1 != null && phone1.isNotEmpty) {
              await _sendSmsNotification(allPaymentsInfo2, phone1);
              await Future.delayed(const Duration(milliseconds: 800));
            }
          }
          ...
          for (final admin in adminUsersClient) {
            if (termAggregation) {
              ...
              adminSmsFutures.add(sendSms(allAdminPaymentsInfo, admin.phone));
            }
          }

// Wait for all admin SMS to complete
