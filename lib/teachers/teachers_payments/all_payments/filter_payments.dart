...
class ViewByScreens extends StatefulWidget {
  const ViewByScreens({Key? key}) : super(key: key);

  @override
  _ViewByScreenState createState() => _ViewByScreenState();
}

class _ViewByScreenState extends State<ViewByScreens> {
  ...
  List<TeacherPaymentsPurposes> filteredPaymentPurposesOnly = [];
  ...
  Future<void> _fetchInitialData() async {
    ...
    final paymentPurposeBox = await Hive.openBox<TeacherPaymentsPurposes>(
  ...
  Future<Uint8List> generateStudentsPDF(
      List<TeacherPayment> studentPayments) async {
    ...
    final grandTotalPaidRow = [
      ...
      pw.Text('', style: headerTextStyle), // Empty cell for 'Assigned Class'
      ..._selectedPaymentPurposes.map((purpose) {
        ...
        build: (pw.Context context) {
          return [
            pw.Column(
              ...
              children: [
                // Title of the page
                pw.Text('Staff Payments Information',
            ...
            pw.Table.fromTextArray(
              headers: [
                ...
                pw.Text('Assigned Class', style: headerTextStyle),
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (confirmSave) {
                ...
                await savePDFToFile(context, pdfBytes, 'Staff_Payments_report');
              }
            },
          ),
          IconButton(
  ...
  Widget _buildPaymentsTable() {
    ...
    for (var entry in groupedPayments.entries) {
      ...
      for (var purposeOnly in _paymentPurposesOnly) {
        final purpose = Hive.box<TeacherPaymentsPurposes>(
                'teacher_payments_purposes')
            .values
            .firstWhere(
              (p) =>
              ...
              orElse: () => TeacherPaymentsPurposes(
                paymentPurpose: 'N/A',
                associatedStaff: [], id: 0,
                purposeAmount: 0.0,
                // Add any other required fields for your `TeacherPaymentsPurposes` model
              ),
            );
        ...
        if (purpose.paymentPurpose != 'N/A') {
          for (var entry in groupedPayments.entries) {
            ...
            // Check if the trimmed studentClass exists in the trimmed associatedStaff list
            final associatedStaffTrimmed = purpose.associatedStaff
                ?.map((staff) => staff.trim()) // Trim each element in the list
                .toList();

            if (associatedStaffTrimmed?.contains(studentClass) ?? false) {
              final purposeAmount = _paymentPurposeAmounts[purposeOnly] ?? 0.0;
              final matchingAmount = entry.value.entries
                      .firstWhere(
                        (e) => e.key.toLowerCase() == purposeOnly.toLowerCase(),
                  ...
                  columns: [
                    DataColumn(
                      label: Container(
                        ...
                        child: Text('STAFF NAME'.toUpperCase()),
                      ),
                    ),
                    DataColumn(
                      label: Container(
                        ...
                        padding: const EdgeInsets.all(8.0),
                        child: Text('ASSIGNED CLASS'.toUpperCase()),
                      ),
                    ),
                    ..._selectedPaymentPurposes.map((purpose) {
                      return DataColumn(
                    ...
                    ..._paymentPurposesOnly.map((purposeOnly) {
                      ...
                      return DataColumn(
                        label: Container(
                          ...
                          padding: const EdgeInsets.all(8.0),
                          child: Text('$purposeOnly BALANCE (\$$purposeAmount)'
                              .toUpperCase()),
                        ),
                      );
                    }),
                    DataColumn(
                      label: Container(
                        ...
                        padding: const EdgeInsets.all(8.0),
                        child: Text('TOTAL BALANCES'.toUpperCase()),
                      ),
                    ),
                  ],
                  rows: [
...
// Handle the default case if necessary
                        ...
                        final associatedStaffTrimmed = purpose.associatedStaff
                            ?.map((staff) =>
                                staff.trim()) // Trim each element in the list
                            .toList();
                        // Check if the student's class is associated with the payment purpose
                        if (!(associatedStaffTrimmed?.contains(studentClass) ??
                            false)) {
  ...
  void generateAndSaveSpreadsheet() async {
    ...
    List<CellValue?> headers = [
      ...
      TextCellValue('ASSIGNED CLASS'),
      ..._selectedPaymentPurposes
          .map((purpose) => TextCellValue('$purpose PAID(\$)')),
      TextCellValue('TOTAL PAID'),
      ..._paymentPurposesOnly
          .map((purposeOnly) => TextCellValue('$purposeOnly BALANCE (\$)')),
      TextCellValue('TOTAL BALANCES'),
    ];
...
// Calculate totals for each purpose
    ...
    final file = File('${directory.path}/staff_payments_report.xlsx');
...
class _ViewByScreenState extends State<ViewByScreens> {
  ...
  Future<void> _fetchInitialData() async {
    ...
    // Fetch unique classes where termId matches globalTermId
    ...
    // Fetch unique payment purposes where termId matches globalTermId
    ...
    // Fetch unique payment purposes from the purposes db where termId matches globalTermId
    _paymentPurposesOnly.addAll(paymentPurposeBox.values
        .where((purposeOnly) => purposeOnly.termId == globalTermId)
        .map((purposeOnly) => purposeOnly.paymentPurpose)
        .toSet()
    ...
    // Fetch payment purpose amounts only where termId matches globalTermId
    for (var purposeOnly
  ...
  Future<void> _fetchStudentsForClass(String studentClass) async {
    ...
    // Fetch unique students for the selected class where termId matches globalTermId
    _students = paymentBox.values
        .where((payment) =>
            payment.studentClass == studentClass &&
            payment.termId == globalTermId)
  ...
  void _filterPayments() {
    ...
    // Filter payments where termId matches globalTermId
    _filteredPayments = paymentBox.values
        .where((payment) => payment.termId == globalTermId)
        .toList();
  ...
  Future<Uint8List> generateStudentsPDF(
    ...
    // Define table headers for both sections
    final headers = [
      pw.Text('Staff Name', style: headerTextStyle),
      pw.Text('Assigned Class', style: headerTextStyle),
      ..._paymentPurposes
      ...
      ..._paymentPurposesOnly
          .map((purposeOnly) =>
              pw.Text('$purposeOnly ARREARS (\$)', style: headerTextStyle))
          .toList(),
      pw.Text('TOTAL ARREARS', style: headerTextStyle),
    ];
    ...
    final dataRows = groupedPayments.entries.map((entry) {
      ...
      // Build row for payments section
      final firstSectionCells = [
        pw.Text(studentName, style: cellTextStyle),
        pw.Text(studentClass, style: cellTextStyle),
          ...
          IconButton(
            ...
            onPressed: () async {
              ...
              if (confirmSave) {
                // Save the PDF after confirmation
                await savePDFToFile(
                    context, pdfBytes, 'detailed_staff_payments_report');
              }
            },
          ),
        ],
                ...
                child: DataTable(
                  columns: [
                    DataColumn(
                      label: Container(
                        color: Colors
                            .greenAccent, // Background color for first section
                        padding: const EdgeInsets.all(8.0),
                        child: Text('STAFF NAME'.toUpperCase()),
                      ),
                    ),
                    ...
                    ..._paymentPurposesOnly.map((purposeOnly) {
                      ...
                      return DataColumn(
                        label: Container(
                          color: const Color.fromARGB(255, 246, 55,
                              2), // Background color for second section
                          padding: const EdgeInsets.all(8.0),
                  ...
                  rows: groupedPayments.entries.map<DataRow>((entry) {
                    final studentName = entry.key;
                    final studentClass = _filteredPayments
                        .firstWhere(
                          (payment) =>
                    ...
                    // Building cells for first section (student payments)
                    List<DataCell> firstSectionCells = [
                      DataCell(Text(studentName)),
                      // Display student class here
                    ];

                    firstSectionCells.addAll(_paymentPurposes.map((purpose) {
                      final amount = entry.value[purpose] ?? 0.0;
                      return DataCell(Text(amount.toStringAsFixed(2)));
                    }));

                    firstSectionCells.add(
                        DataCell(Text(totalPaidAmount.toStringAsFixed(2))));

                    // Building cells for second section (purpose amounts from database)
                    List<DataCell> secondSectionCells =
                        _paymentPurposesOnly.map((purposeOnly) {
                      final purposeAmount =
                          _paymentPurposeAmounts[purposeOnly] ?? 0.0;
                      ...
                      return DataCell(Text(result.toStringAsFixed(2)));
                    }).toList();

                    // Calculate total arrears for the row
                    final totalArrears = secondSectionCells
                        .map((cell) =>
                            double.tryParse(cell.child is Text
                                ? (cell.child as Text).data ?? '0'
                                : '0') ??
                            0.0)
                        .reduce((a, b) => a + b);

                    // Add total arrears cell
                    secondSectionCells
                        .add(DataCell(Text(totalArrears.toStringAsFixed(2))));

                    // Combine both sections
                    return DataRow(
                        cells: firstSectionCells + secondSectionCells);
                  }).toList(),
                ),
              ),
            ),
