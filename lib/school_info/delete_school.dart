...
class _DeleteSchoolScreenState extends State<DeleteSchoolScreen> {
  ...
  List<School> _foundSchools = []; // List of found schools for display

  void _searchSchool() async {
    final box = await Hive.openBox<School>('school'); // Open School Hive box
    ...
    final schoolsWithName = schools
        .where((school) =>
            school.schoolName!.toLowerCase().startsWith(searchTerm) &&
            school.termId != null)
        .toList();

    // Sort alphabetically by school name
    schoolsWithName
        .sort((a, b) => a.schoolName!.compareTo(b.schoolName.toString()));
  ...
  void _deleteSchool(School schoolToDelete) async {
    final box = await Hive.openBox<School>('school');
    if (schoolToDelete.termId != null) {
      await box
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('School cannot be deleted')),
      );
    }
  }
  ...
  void _deleteAllSchools() async {
    ...
    final schoolsToDelete =
        box.values.cast<School>().where((s) => s.termId != null).toList();
                ...
                children: [
                  TextFormField(
                    ...
                    decoration: InputDecoration(
                      labelText: 'Enter School Name to Search',
                      filled: true,
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Card(
                            ...
                            child: ListTile(
                              ...
                              title: Text('Name: ${foundSchool.schoolName}',
                                  style: const TextStyle(
                                      fontSize: 18,
                                      fontWeight: FontWeight.bold)),
                              subtitle: Text('Term: ${foundSchool.termId}',
