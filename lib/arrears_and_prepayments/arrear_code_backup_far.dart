...
class _ViewByScreenState extends State<ArrearsAndPrepayments> {
  ...
  // termId -> purposeName -> totalPaid
  final Map<String, Map<String, double>> termPaidByPurpose = {};

// termId -> totalPaid across all purposes
  final Map<String, double> termTotalPaid = {};
...
// Same for _selectedPaymentPurposes
    ...
    List<CellValue?> headers = [
      ...
      ...normalizedPaymentPurposesOnly.map((p) =>
          TextCellValue('$p ARREARS (\$${_paymentPurposeAmounts[p] ?? 0.0})')),
    ...
    for (var entry in allStudentsMap.entries) {
      ...
      final List<DataCell> arrearsCells =
        ...
        if (isExceptional) {
          arrears = getAdjustedArrear(
              arrears, student, purpose, _selectedTermIds.first);
        }
      ...
      final List<DataCell> paidCells = _selectedPaymentPurposes.map((purpose) {
        final paidAmount = studentPaymentData[purpose] ?? 0.0;
        grandTotalPurposePaid[purpose] =
            (grandTotalPurposePaid[purpose] ?? 0.0) + paidAmount;
        return DataCell(Text(paidAmount.toStringAsFixed(2)));
      }).toList();
      sheetObject.appendRow([
        ...
        ...arrearsCells.map((arrear) {
          final widget = arrear.child;
          if (widget is Text) {
            return TextCellValue(
              (double.tryParse(widget.data ?? '') ?? 0.0) > 0
                  ? '-${widget.data}'
                  : (widget.data ?? ''),
            );
          }
          return TextCellValue(''); // fallback
        }),
        TextCellValue(
          totalArrears > 0.0
              ? '-${totalArrears.abs().toStringAsFixed(2)}'
              : '0.0',
        ),
    ...
    sheetObject.appendRow([
      ...
      ...normalizedPaymentPurposesOnly.map((purpose) => TextCellValue(
            ((grandTotalPurposeArrears[purpose] ?? 0.0) > 0.0)
                ? '-${(grandTotalPurposeArrears[purpose] ?? 0.0).abs().toStringAsFixed(2)}'
                : '0.0',
          )),
