...
class _UpdateStudentScreenState extends State<UpdateStudentScreen> {
  ...
  Future<void> _loadTerms() async {
    ...
    if (_role == DeviceRole.host) {
      ...
      setState(() {
        _allTerms = sorted; // ✅ assign all terms
  ...
  Future<void> _searchStudentsDeep(String query) async {
    if (!_roleReady) return;

    // HOST → local Hive search (optional)
    if (_role == DeviceRole.host) {
      final box = await Hive.openBox<Student>('students');
      final students = box.values.toList();

      setState(() {
        _matchingStudents = students.where((s) {
          final q = query.toLowerCase();
          return s.surname.toLowerCase().contains(q) ||
              s.name.toLowerCase().contains(q) ||
              (s.studentIdNumber ?? '').toLowerCase().contains(q);
        }).toList();
      });
      return;
    }
...
// Ensure _selectedTerms is assigned a valid list (handle null case)
  ...
  Future<void> _loadClassTerms(String className) async {
    ...
    if (selectedClass.id != -1) {
      final classTermIds = selectedClass.terms ?? [];

      setState(() {
        // Show all terms for the class, sorted
        _availableTerms = _allTerms
            .where((term) => classTermIds.contains(term.termId))
            .toList();

        // ❌ Do NOT override _selectedTerms here — leave the student's saved terms intact
        // _selectedTerms = ...
      });
    }
  }
      ...
      child: Form(
        ...
        child: ListView(
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                final query = _surnameController.text.trim();
                if (query.isNotEmpty) {
                  _searchStudentsDeep(query);
                }
              },
              style: ElevatedButton.styleFrom(
                padding: const EdgeInsets.symmetric(vertical: 16),
            ...
            if (_foundStudent != null) ...[
              ...
              const SizedBox(height: 20),
              ElevatedButton(
                onPressed: _foundStudent == null
                    ? null
                    : () {
                        if (_role == DeviceRole.host) {
                          // Only host can update
                          _updateStudent(_foundStudent!);
                        } else {
                          // Client tries → show notification
                          _showDialog(
                            'Updating student records is only allowed on the host device Now.',
                          );
                        }
                      },
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color.fromARGB(255, 225, 232, 243),
                  padding: const EdgeInsets.symmetric(vertical: 16),
                ),
                child: const Text(
                  'Update Student',
  ...
  Widget _buildTermSelection() {
    return _availableTerms.isEmpty
        ...
        : Column(
            children: _availableTerms.map((term) {
              return CheckboxListTile(
                title: Text(term.termName ?? 'Unnamed Term'), // display name
                value: _selectedTerms.contains(term.termId), // check by ID
                onChanged: (selected) {
                  setState(() {
                    if (selected == true) {
                      if (!_selectedTerms.contains(term.termId)) {
