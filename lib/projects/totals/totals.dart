...
class _ViewByScreenState extends State<Totals> {
  ...
  List<ProjectItem> filteredPaymentPurposesOnly = [];
  ...
  Future<void> _fetchInitialData() async {
    ...
    final paymentPurposeBox = await Hive.openBox<ProjectItem>('projectItems');

    final filteredPayments = paymentBox.values.toList();

    final filteredPaymentPurposesOnly = paymentPurposeBox.values.toList();
    ...
    _classes.addAll(filteredPayments
        .map((student) => student.projectCode)
        .toSet()
        .toList());
    ...
    _paymentPurposesOnly.addAll(paymentPurposeBox.values
        .where((purposeOnly) => purposeOnly.projectItemCode != null)
        .map((purposeOnly) => purposeOnly.projectItemCode)
        .toSet()
        .toList());

    // Fetch payment purpose only amounts
    for (var purposeOnly in paymentPurposeBox.values) {
      _paymentPurposeAmounts[purposeOnly.projectItemCode] = purposeOnly.amount;
    }
  ...
  Future<void> _fetchStudentsForClass(String studentClass) async {
    ...
    _students = paymentBox.values
        .where((payment) => payment.projectCode == studentClass)
  ...
  void _filterPayments() {
    ...
    if (_selectedPaymentPurposes.isNotEmpty &&
      ...
      _filteredPayments = _filteredPayments.where((payment) {
        return _selectedPaymentPurposes.contains(payment.projectStudentPaymentCode);
      }).toList();
    }
    ...
    if (_selectedPaymentPurposesArrears.isNotEmpty &&
      ...
      filteredPaymentPurposesOnly =
        ...
        return _selectedPaymentPurposes.contains(payment.projectItemCode) &&
            (payment.amount < 0.0);
      }).toList();
    }
    ...
    if (_selectedSortOption == 'Surname') {
      _filteredPayments.sort((a, b) => a.studentId
          .toLowerCase()
          .compareTo(b.studentId.toLowerCase()));
  ...
  Future<Uint8List> generateStudentsPDF(
      List<ProjectStudentPayment> studentPayments) async {
  ...
  Widget _buildPaymentsTable() {
    ...
    for (var entry in groupedPayments.entries) {
      ...
      for (var purposeOnly in _paymentPurposesOnly) {
        final purpose = Hive.box<ProjectItem>('projectItems').values.firstWhere(
              (p) =>
              ...
              orElse: () => ProjectItem(
                ...
                purposeAmount: 0.0,
                // Add any other required fields for your `ProjectItem` model
              ),
            );
                ...
                child: DataTable(
                  ...
                  rows: [
                    ...groupedPayments.entries.map((entry) {
                      ...
                      final arrearsCells =
                          _paymentPurposesOnly.map((purposeOnly) {
                        final purpose = Hive.box<ProjectItem>('projectItems')
                            .values
                            .firstWhere(
                              (p) =>
