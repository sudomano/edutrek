...
// bluetooth helper
  ...
  void _searchStudent() async {
    ...
    if (_role == DeviceRole.host) {
      ...
      if (_cachedServerStudents == null) {
        _showDialog('⚠️ Students not loaded yet. Please wait.');
        return;
      }
    ...
    if (matchingStudents.isEmpty) {
      _showDialog('No matching students found for this Term: ($globalTermId)');
    } else {
      _paymentInfo = '';
      _paymentInfo11 = '';
      _paymentInfo1 = '';
  ...
  Future<double> _computeTotalStudentArrears(Student student) async {
    ...
    try {
      ...
      for (final entry in arrearPurposes) {
        final PaymentPurpose purpose = entry['purpose'];
        final arrearsData = await _computeArrearsForPurpose(purpose);

        // Sum only positive arrears
        for (final amount in arrearsData.values) {
...
// Fetch payment purposes by termId
  ...
  /*
                                              ...
                                              items: _arrearsDetails.entries
    .where((entry) => entry.value > 0.0)
    .map((entry) {
      return DropdownMenuItem<String>(
        value: entry.key,
        child: Text('Term: ${entry.key} - Arrears: \$${entry.value.toStringAsFixed(2)}'),
      );
    }).toList(),
*/
