...
class _MyPageState extends State<HomeScreen> {
  ...
  Future<User> getLoggedInUser() async {
    if (!Hive.isBoxOpen('users')) {
      await Hive.openBox<User>('users'); // Ensure box is opened
    }
    final box = await Hive.openBox<User>('users');
    final user = box.getAt(0);
    if (user == null) {
      throw HiveError("No logged-in user found in the 'users' box.");
    }
    return user;
  }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<User>(
      future: getLoggedInUser(), // Uses async-safe version
      builder: (context, snapshot) {
        ...
        final administration =
            loggedInUser.role.toLowerCase() == 'administration';
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            backgroundColor: const Color.fromARGB(
                255, 38, 140, 191), // AppBar background color
